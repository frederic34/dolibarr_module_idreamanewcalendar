...
switch ($action) {
	...
	case 'getevents':
		try {
			...
			dol_syslog(print_r($parameters, true), LOG_DEBUG);
		} catch (Exception $e) {
			dol_syslog('json_decode error', LOG_ERR);
		}
		...
		print json_encode((array) $arrayofevents);
		break;
...
function getEvents($calendarId, $calendarName, $startDate, $endDate, $offset, $onlylast, $search_actioncode, $search_userid, $socid, $search_states, $search_all)
{
	...
	if ($calendarId == '1') {
		...
		if (!empty($conf->global->IDREAMANEWCALENDAR_DONT_SHOW_AUTO_EVENTS) && strpos(implode(',', $actioncode), 'AC_OTH_AUTO') === false) {
			// a.code au lieu de ca.code
	...
	if (count($listofextcals)) {
		...
		$cachedir = DOL_DATA_ROOT . '/idreamanewcalendar/temp';
...
function isDarkColor($color)
{
	...
	$lightness_swap = empty($conf->global->IDREAMANEWCALENDAR_LIGTHNESS_SWAP) ? 150 : $conf->global->IDREAMANEWCALENDAR_LIGTHNESS_SWAP;
